...
<div class="relative pb-[56.25%] h-0 overflow-hidden video-player-wrapper flex justify-start items-start"
    ...
    <div class="absolute inset-0 z-10 pointer-events-none" style="background: transparent;"></div>
    <div class="absolute z-20 pointer-events-none select-none text-sm sm:text-xs"
        style="top: 50%; left: 15px; color: rgba(255,255,255,0.7); font-weight: bold; text-shadow: 2px 2px 4px rgba(0,0,0,0.8);">
        {{ $user->name }} ({{ $user->email }} - {{ $user->phone }})
    </div>
    <div class="absolute z-20 pointer-events-none select-none text-sm sm:text-xs"
        style="top: 15px; right: 15px; color: rgba(255,255,255,0.4); text-shadow: 1px 1px 2px black;">
        IP: {{ request()->ip() }}
    </div>
